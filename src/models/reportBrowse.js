...
export default modelExtend(pageModel, {
  ...
  effects: {
    ...
    * getModels({payload}, { call, put, select }){
        const models = yield select(_ => _.reportBrowse.models);
        if (models.length) {
        } else {
    ...
    * getInstances({payload}, { call, put, select }){
        const instances = yield select(_ => _.reportBrowse.models);
        if (instances.length) {
        } else {
    ...
    * getTaskListModel({payload}, { call, put, select }){
      const tasks = yield select(_ => _.reportBrowse.tasks);
      if (tasks.length) {
      } else {
    ...
    *listInstanceByNameModel({ payload }, { call, put, select }) {
      const chooseReport = yield select(_ => _.reportBrowse.chooseReport);
      const data = yield call(listInstanceByNameService, {
      ...
      if (data.result === '0') {
        ...
        if (result && result.length == 0) {
          ...
          result = result && result.map(item => ({ name: item.instanceName, value: item.instanceId}))
          yield put({
            type: 'setState',
            payload: {
              instances: result,
